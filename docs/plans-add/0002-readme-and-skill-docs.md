...
- **add-knowhow**: 実装知見の記録・蓄積
- **add-knowhow-index**: knowhow インデックスの読み込み・再構築
- **add-knowhow-filter**: Plan に関連する knowhow のフィルタリング
- **add-dev-loop**: 自律タスク選択・実装ループ
- **add-experience**: スキル経験ファイルの検証
- **add-gui-test**: GUI テスト実行（オプション）
- **add-annotate-grid**: 画像グリッドアノテーション
- **add-clip-image**: 画像領域切り出し
...
- コードレビュー Warning 3件を修正:
  - スキル名 `dev-loop` → `add-dev-loop` に統一
